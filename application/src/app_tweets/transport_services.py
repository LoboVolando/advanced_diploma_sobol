...
from .schemas import TweetSchema, AuthorSchema, TweetInSchema, TweetOutSchema, MeAuthorSchema, MeAuthorOutSchema
...
class TweetMockService(AbstractTweetService):
    ...
    async def get_list(self) -> t.Optional[t.List[TweetSchema]]:
        return [
            TweetSchema(id=1, content="tweet-1", author=AuthorSchema(id=1, name="Николай Носов")),
            TweetSchema(id=2, content="tweet-2", author=AuthorSchema(id=1, name="Николай Гоголь")),
        ]
...
class AuthorMockService(AbstractAuthorService):
    async def me(self, api_key: str) -> MeAuthorOutSchema:
        user = MeAuthorOutSchema(result=True,
                                 user=MeAuthorSchema(
    ...
    def _get_authors_list(self, count: int):
        return [AuthorSchema(id=random.randint(1, 300000), name=f'Author_{i}') for i in range(count)]
